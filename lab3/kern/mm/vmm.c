...
struct mm_struct *
mm_create(void) {
    //使用 kmalloc 为 mm_struct 分配内存
    struct mm_struct *mm = kmalloc(sizeof(struct mm_struct));
    //如果分配成功，则初始化 mm_struct 的各个字段
    if (mm != NULL) {
        //调用 list_init 初始化 mmap_list 链表，用于存储该地址空间中的所有 VMA
        list_init(&(mm->mmap_list));
        mm->mmap_cache = NULL;
        //pgdir 是指向页目录表的指针
        mm->pgdir = NULL;
        mm->map_count = 0;
        //检查全局变量 swap_init_ok，用于判断页面置换机制是否已经初始化。
...
do_pgfault(struct mm_struct *mm, uint_t error_code, uintptr_t addr) {
    //指向当前进程的 mm_struct，管理其虚拟地址空间。error_code：指明引发页错误的具体原因（如读写权限问题）。
//addr：引发页错误的虚拟地址。
    // pte_t* temp = NULL;
...
//查找地址 addr 对应的页表项指针
    ptep = get_pte(mm->pgdir, addr, 1);  //(1) try to find a pte, if pte's
                                         ...
                                         //create a PT.
    if (*ptep == 0) {
        //如果页表尚未分配，get_pte 会根据第三个参数创建新的页表
        //调用 pgdir_alloc_page 分配一个新物理页面并将其映射到 addr
