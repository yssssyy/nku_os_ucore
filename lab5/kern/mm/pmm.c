...
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end, bool share) {
    ...
    // 确保给定的内存范围是在用户空间地址范围内
    assert(USER_ACCESS(start, end));

    // 按照页面单位复制内存内容
    do {
        // 获取源进程的页表项 ptep，指向起始地址 start 对应的物理页
        pte_t *ptep = get_pte(from, start, 0), *nptep;

        // 如果源进程的页表项不存在，跳过此页，继续处理下一个页面
        if (ptep == NULL) {
            start = ROUNDDOWN(start + PTSIZE, PTSIZE); // 将 start 地址对齐到下一页
            continue;
        }

        // 获取目标进程的页表项 nptep，如果目标进程的页表项不存在，分配一个新的页表
        if (*ptep & PTE_V) {  // 检查源页表项是否有效（即该页面存在）
            if ((nptep = get_pte(to, start, 1)) == NULL) { // 如果目标页表项不存在，分配一个
                return -E_NO_MEM; // 如果分配失败，返回内存不足的错误
            }

            uint32_t perm = (*ptep & PTE_USER); // 获取源页面的权限，是否为用户访问
            // 获取源进程页面的物理页面对象
            ...
            assert(npage != NULL); // 确保目标页面分配成功

            int ret = 0;
            /* LAB5:EXERCISE2 YOUR CODE
             * 将源页面的内容复制到目标页面 npage，并为目标页面构建映射。
             * 
             * 下面的代码将会实现：
            ...
            memcpy(dst, src, PGSIZE); // 复制页面内容，复制的大小为一页（PGSIZE）

            // 将新分配的目标页面插入目标进程的页表中，建立物理地址映射
            ret = page_insert(to, npage, start, perm);

            assert(ret == 0); // 确保映射插入成功
        }
        start += PGSIZE; // 处理下一个页面
    } while (start != 0 && start < end); // 当处理的地址未超过结束地址时，继续循环
