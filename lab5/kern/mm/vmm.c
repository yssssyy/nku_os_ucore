...
int dup_mmap(struct mm_struct *to, struct mm_struct *from) {
    ...
    while ((le = list_prev(le)) != list) {
        ...
        // 初始化标志，判断是否共享内存
        bool share = 0;//share 变量的值为 0 表示 不共享内存，即目标进程将获得源进程内存区域的独立副本。

        // 调用 copy_range 函数来复制内存区域的内容
        // 如果复制失败，返回内存不足的错误码
...
do_pgfault(struct mm_struct *mm, uint_t error_code, uintptr_t addr) {
    ...
    // 判断页表项权限，如果有效但是不可写，跳转到COW处理
    if ((ptep = get_pte(mm->pgdir, addr, 0)) != NULL) {  // 获取进程页表中的页表项
    // 检查页表项是否有效（PTE_V）且不可写（~PTE_W）
        if ((*ptep & PTE_V) & ~(*ptep & PTE_W)) {
        // 如果页表项有效但是不可写，调用 cow_pgfault 进行写时复制处理
